...
void Mac1609_4::handleSelfMsg(cMessage* msg) {
    ...
    if(checkFailSafe())
    {
        std::cout << "**************** Fail-safe activating @ self .. ************************" << endl;
        return;
    }
	...
	if (msg == nextChannelSwitch) {
		...
		if ((!useSCH) || (timeLeftInSlot() > sendingDuration)) {
			...
			mac_delay_diff = (simTime() - pktToSend->getTimestamp()).dbl() * 1000000;
			mac_delay.push_back(mac_delay_diff);
...
double Mac1609_4::delayStdev()
{
    ...
    std::transform(mac_delay.begin(), mac_delay.end(), diff.begin(), std::bind2nd(std::minus<double>(), mean));
...
/* checks if vehicle is misbehaving in terms of broadcasting */
bool Mac1609_4::checkFailSafe() {
    ...
    if(delayMean() > 50 && mac_delay.size() > 10) // hard coded value for now without stdev
        score = score + 0.2;

    if(numBackoff > 300)
        std::cout << "Fail-safe rb : " << rb << " Numbackoff " << numBackoff << " Mean: " << delayMean() << " Score: "<< score << endl;
...
void Mac1609_4::handleUpperMsg(cMessage* msg) {
	...
	// Let's check continuously for Fail-Safe

	if(checkFailSafe())
	{
	    std::cout << "**************** Fail-safe activating.. ************************" << endl;
	    return;
	}

	thisMsg->setTimestamp(simTime()); // for MAC processing delay
...
void Mac1609_4::finish() {
	...
	double mac_delay_avg = accumulate( mac_delay.begin(), mac_delay.end(), 0.0)/mac_delay.size();
...
bool Mac1609_4::guardActive() const {
    ...
    if(fail_safe)
    {
        std::cout << "**************** Fail-safe activating @ guard.. ************************" << endl;
        return true;
    }
